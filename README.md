...
| **US-10** | **Sincronización Biométrica** | Como Medical Staff, quiero sincronizar mi wearable para enviar mis métricas de estrés al motor de evaluación. | **Escenario 1**<br>**Given** el wearable emparejado por bluetooth,<br>**When** ejecuto SyncBiometricData,<br>**Then** el sistema de riesgo recibe la telemetría fisiológica en tiempo real.<br><br>**Escenario 2**<br>**Given** una desconexión momentánea de red en el hospital,<br>**When** la aplicación recupera la cobertura,<br>**Then** sincroniza de inmediato todos los datos que fueron guardados de forma local en la caché. | **EP03** |
...
| **US-24** | **Redistribución Automática** | Como sistema, quiero equilibrar la carga laboral cuando hay ausencias críticas. | **Escenario 1**<br>**Given** múltiples bloqueos de turnos en un mismo departamento,<br>**When** actúa la WorkloadRedistributionPolicy,<br>**Then** actualiza el Workload Distribution View para prevenir la fatiga en los suplentes.<br><br>**Escenario 2**<br>**Given** un médico de reemplazo que ya ha alcanzado el límite máximo de horas de guardia semanales,<br>**When** se balancea la carga,<br>**Then** el sistema lo omite automáticamente de las recomendaciones. | **EP05** |
...
| **US-27** | **Notificación de Recuperación** | Como sistema, quiero avisar al personal médico que se le ha asignado un plan de recuperación. | **Escenario 1**<br>**Given** un plan en estado propuesto,<br>**When** actúa la RecoveryNotificationPolicy,<br>**Then** ejecuta NotifyMedicalStaff mostrando una alerta prioritaria en el terminal del médico.<br><br>**Escenario 2**<br>**Given** que el usuario abre la aplicación móvil,<br>**When** se dirige a la sección de pausas,<br>**Then** el Recovery Recommendation View renderiza el temporizador y las instrucciones de la pausa activa. | **EP06** |
...
| **US-34** | **Sincronización del Historial** | Como sistema, quiero actualizar de inmediato las vistas de auditoría cuando se agregan nuevos registros. | **Escenario 1**<br>**Given** nuevos comandos validados y escritos en el Aggregate de control,<br>**When** actúa la AuditTrailSynchronizationPolicy,<br>**Then** se actualiza en tiempo real la interfaz del Audit Log View.<br><br>**Escenario 2**<br>**Given** una consulta abierta al historial por parte de gerencia,<br>**When** entran nuevos registros de incidentes,<br>**Then** la vista ordena la información basándose en milisegundos de ocurrencia sin necesidad de recargar. | **EP07** |
| **US-35** | **Generación de Reporte de Cumplimiento** | Como Administrador Institucional, quiero generar reportes de salud ocupacional para entidades regulatorias. | **Escenario 1**<br>**Given** un rango mensual de búsqueda seleccionado,<br>**When** ejecuto GenerateComplianceReport,<br>**Then** el sistema compila los datos consolidados y los expone en el Compliance Report View.<br><br>**Escenario 2**<br>**Given** la información renderizada en pantalla,<br>**When** doy clic en el botón de descarga,<br>**Then** el sistema exporta un documento PDF estructurado y listo para auditorías institucionales de seguridad. | **EP07** |
...
| **US-39** | **Solicitud de Acceso Especial** | Como Medical Staff, quiero solicitar acceso a módulos de supervisión temporalmente si me delegan dicha responsabilidad. | **Escenario 1**<br>**Given** que cuento únicamente con rol operativo de staff,<br>**When** ejecuto el comando RequestAccess para una sala crítica,<br>**Then** el sistema envía una solicitud de aprobación en tiempo real al panel administrativo.<br><br>**Escenario 2**<br>**Given** que el administrador valida positivamente mi petición,<br>**When** se procesa la actualización del estado,<br>**Then** la política me asigna el rol de supervisor auxiliar de forma temporal dentro de mi UserAccount. | **EP02** |
...
<p>
  Los diagramas de clases del backend representan la estructura interna del lado del servidor. 
  Describen los modelos de dominio, controladores, servicios de aplicación, repositorios, objetos de valor y relaciones que soportan la lógica de negocio de cada bounded context.
</p>
...
<p>
  ...
  Incluye las entidades, servicios y repositorios responsables de los registros de auditoría, verificaciones de cumplimiento, reportes regulatorios y trazabilidad del sistema.
</p>
...
<p>
  ...
  Incluye las entidades y servicios responsables de procesar datos biométricos, detectar anomalías, evaluar el riesgo clínico y apoyar la toma de decisiones médicas.
</p>
...
<p>
  ...
  Incluye entidades de dominio, servicios, controladores y repositorios responsables de la autenticación, registro de usuarios, invitaciones, roles y permisos de acceso.
</p>
...
<p>
  ...
  Muestra las entidades, servicios y repositorios involucrados en alertas clínicas, acciones preventivas, escalamiento de incidentes y seguimiento de respuestas.
</p>
...
<p>
  ...
  Incluye la lógica de dominio relacionada con turnos médicos, gestión de horarios, asignaciones de turno y coordinación entre el personal médico y los supervisores.
</p>
...
<p>
  ...
  Incluye las entidades y servicios responsables de planes de recuperación, descansos, recomendaciones y acciones para reducir la fatiga del personal médico.
</p>
...
<p>
  ...
  Incluye la lógica de dominio relacionada con planes, suscripciones, pagos, estado de facturación y limitaciones de acceso según el plan activo de la organización.
</p>
...
<p>
  ...
  Incluye elementos comunes del dominio, entidades base, objetos de valor, excepciones compartidas y estructuras reutilizables entre distintos bounded contexts.
</p>
