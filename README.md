...
| --- | --- | --- | --- |
| **EP01** | **Gestión de Suscripciones y Planes** | Como administrador institucional, quiero gestionar los planes de pago y la activación de suscripciones para habilitar las funcionalidades contratadas por el hospital. | **Escenario 1: Activación tras confirmación de pago**<br>**Given** que el administrador ha completado el pago de un plan,**When** Stripe Sandbox confirma la transacción exitosa,**Then** la plataforma cambia el estado de la suscripción a 'Activa' y notifica al usuario.<br><br>**Escenario 2: Restricción automática por expiración**<br>**Given** que la suscripción de la institución ha llegado a su fecha de vencimiento sin renovación,**When** un usuario intenta ingresar a los tableros del hospital,**Then** el sistema blocks el acceso a las herramientas premium y muestra una alerta de renovación de plan. |
| **EP02** | **Gestión de Identidad y Accesos** | Como usuario de la plataforma, quiero registrarme, autenticarme y recibir roles dentro del sistema para acceder de manera segura a mi espacio de trabajo hospitalario. | **Escenario 1: Verificación de usuario invitado**<br>**Given** que un profesional médico recibe un correo de invitación,**When** acepta los términos y completa su registro a través de Firebase Auth,**Then** el sistema lo asocia correctamente al workspace de su hospital con su estado activo.<br><br>**Escenario 2: Denegación por permisos insuficientes**<br>**Given** que un usuario con el rol 'Medical Staff' intenta ingresar a la configuración de la organización,**When** el sistema evalúa su perfil,**Then** la política de accesos deniega la entrada y registra un intento no autorizado en el sistema. |
| **EP03** | **Evaluación de Riesgo Clínico** | Como sistema, quiero procesar los datos biométricos del personal médico para calcular su puntaje de fatiga e identificar riesgos clínicos inminentes. | **Escenario 1: Cálculo en tiempo real**<br>**Given** que el flujo de datos biométricos del wearable se está transmitiendo activamente,**When** el motor de análisis aplica la política de evaluación,**Then** actualiza el puntaje de fatiga y refresca el nivel de riesgo en el panel del usuario.<br><br>**Escenario 2: Identificación de riesgo extremo**<br>**Given** que un médico presenta variabilidad cardíaca y cortisol fuera de los umbrales tolerables,**When** se excede el límite crítico preestablecido,**Then** el sistema emite un evento de alerta por riesgo clínico extremo. |
| **EP04** | **Gestión de Incidentes y Escalamiento** | Como supervisor clínico, quiero que el sistema registre incidentes de riesgo y me alerte de forma inmediata para poder mitigar el agotamiento del personal. | **Escenario 1: Apertura automática de incidentes**<br>**Given** que el motor de riesgo detecta una anomalía fisiológica grave en un médico,**When** se dispara el evento de riesgo clínico,**Then** el sistema abre automáticamente un incidente formal y le asigna prioridad alta.<br><br>**Escenario 2: Notificación al supervisor**<br>**Given** un nuevo incidente de prioridad crítica abierto en el sistema,**When** se ejecuta la política de despacho,**Then** se envía una alerta push/email inmediata al supervisor en turno utilizando Resend Email API. |
| **EP05** | **Coordinación de Turnos** | Como supervisor clínico, quiero evaluar turnos críticos, bloquear asignaciones peligrosas y coordinar reemplazos para salvaguardar al personal y asegurar la continuidad operativa. | **Escenario 1: Bloqueo preventivo de turno**<br>**Given** que un médico programado para un turno nocturno entra en estado de riesgo extremo,**When** se evalúa la asignación del turno,**Then** el sistema bloquea automáticamente dicho turno para evitar incidentes laborales.<br><br>**Escenario 2: Proposición automática de suplentes**<br>**Given** que un turno ha sido bloqueado de forma preventiva,**When** el supervisor consulta las alternativas disponibles,**Then** el sistema muestra una lista filtrada de médicos con bajo nivel de fatiga aptos para el reemplazo. |
| **EP06** | **Recuperación del Personal** | Como profesional médico o supervisor, quiero recibir y gestionar recomendaciones de descanso para recuperarme fisiológicamente de la fatiga. | **Escenario 1: Notificación de descanso sugerido**<br>**Given** que se ha detectado la necesidad imperativa de un descanso para un médico,**When** el supervisor aprueba la pausa activa,**Then** el sistema envía una alerta de recuperación directamente a la aplicación del profesional.<br><br>**Escenario 2: Seguimiento de rechazo de plan**<br>**Given** que un médico decide denegar la recomendación de pausa por una urgencia,**When** presiona la opción de rechazar plan,**Then** el sistema cambia el estado del plan de recuperación a 'Rechazado' y levanta un punto de fricción para su revisión. |
| **EP07** | **Auditoría y Cumplimiento** | Como administrador institucional, quiero que el sistema guarde trazabilidad inmutable de todas las evaluaciones de riesgo y decisiones críticas para generar reportes de cumplimiento. | **Escenario 1: Sincronización inmutable del historial**<br>**Given** que un supervisor toma una decisión crítica sobre un turno o incidente,**When** la acción se completa con éxito,**Then** el sistema registra de forma automática e inmediata el evento en el log de auditoría.<br><br>**Escenario 2: Consolidación de reportes institucionales**<br>**Given** el administrador necesita presentar evidencia ante un comité de salud ocupacional,**When** solicita el reporte mensual de intervenciones,**Then** el sistema procesa el historial y genera un informe de cumplimiento inalterable con estadísticas clave. |
...
| --- | --- | --- | --- | --- |
| **US-01** | **Registro de Administrador Institucional** | Como director del hospital, quiero registrar mi cuenta inicial para crear el HospitalWorkspace de mi institución. | **Escenario 1**<br>**Given** datos corporativos válidos,**When** ejecuto RegisterInstitutionalAdministrator,**Then** Firebase Auth crea la cuenta y genera el workspace de la institución.<br><br>**Escenario 2**<br>**Given** un correo que ya está registrado en el sistema,**When** intento registrar la organización,**Then** el sistema deniega la acción por duplicidad de credenciales. | **EP02** |
| **US-02** | **Invitación a Personal Médico** | Como Administrador, quiero invitar a médicos al workspace para que puedan integrarse al monitoreo. | **Escenario 1****Given** el email válido de un médico,**When** ejecuto InviteUser,**Then** se despacha un correo de invitación formal utilizando la Resend Email API.**Escenario 2****Given** un email con formato de sintaxis incorrecto,**When** intento enviar la invitación,**Then** el sistema muestra un error de validación en la interfaz. | **EP02** |
| **US-03** | **Aceptación de Invitación** | Como médico invitado, quiero aceptar la invitación para completar mi registro institucional. | **Escenario 1****Given** que recibo el enlace en mi bandeja,**When** ejecuto AcceptInvitation y configuro mi perfil,**Then** mi UserAccount se vincula exitosamente al workspace.**Escenario 2****Given** un enlace de invitación expirado en el tiempo,**When** intento acceder,**Then** el sistema rechaza la acción y me ofrece solicitar un nuevo envío. | **EP02** |
| **US-04** | **Asignación de Roles** | Como Administrador, quiero asignar roles específicos (Clinical Supervisor, Medical Staff) para delimitar accesos. | **Escenario 1****Given** un usuario activo en el sistema,**When** ejecuto AssignUserRole con el rol de 'Clinical Supervisor',**Then** la RoleBasedAccessPolicy le otorga acceso inmediato a los reportes clínicos corporativos.**Escenario 2****Given** que intento remover el único rol administrativo de la cuenta raíz,**When** guardo cambios,**Then** el sistema bloquea la acción para evitar la inactividad del espacio. | **EP02** |
| **US-05** | **Autenticación Segura** | Como usuario, quiero iniciar sesión mediante Firebase Auth para acceder a mis paneles correspondientes. | **Escenario 1****Given** credenciales de acceso correctas,**When** ejecuto SignIn,**Then** el sistema valida mi rol institucional y me redirige a mi vista (Read Model) asignada.**Escenario 2****Given** 5 intentos fallidos consecutivos de inicio de sesión,**When** intento loguearme de nuevo,**Then** Firebase bloquea temporalmente la cuenta por seguridad. | **EP02** |
| **US-06** | **Selección de Plan de Suscripción** | Como Administrador, quiero elegir el nivel de suscripción para habilitar las funcionalidades necesarias para mi hospital. | **Escenario 1****Given** que exploro las opciones comerciales,**When** ejecuto SelectSubscriptionPlan (Enterprise),**Then** el Aggregate Subscription actualiza el plan y queda en espera del pago.**Escenario 2****Given** que tengo un plan activo actualmente,**When** selecciono un plan inferior,**Then** el sistema programa el downgrade automático para que aplique en el siguiente ciclo de facturación. | **EP01** |
| **US-07** | **Confirmación de Pago** | Como Administrador, quiero procesar el pago para activar oficialmente el servicio. | **Escenario 1****Given** una tarjeta de prueba válida en Stripe Sandbox,**When** ejecuto ConfirmSubscriptionPayment,**Then** la PaidSubscriptionConfirmedPolicy cambia el estado a 'Activo'.**Escenario 2****Given** una tarjeta declinada o sin fondos,**When** Stripe procesa el cargo,**Then** el sistema retorna un error en pantalla y la suscripción permanece en 'Pendiente de Pago'. | **EP01** |
| **US-08** | **Habilitación de Funcionalidades** | Como sistema, quiero habilitar características específicas basándome en el plan pagado. | **Escenario 1****Given** el evento SubscriptionActivated,**When** el sistema evalúa el plan contratado,**Then** la ActiveSubscriptionFeaturePolicy desbloquea los módulos Premium institucionales.**Escenario 2****Given** un usuario en plan Básico,**When** intenta dar clic en el módulo de Audit Trail avanzado,**Then** el sistema muestra un modal recomendando un Upgrade de plan. | **EP01** |
| **US-09** | **Restricción por Expiración** | Como sistema, quiero restringir accesos si el periodo de suscripción ha culminado sin renovación. | **Escenario 1****Given** que la fecha límite de pago pasó sin renovación,**When** un usuario intenta ingresar a los paneles,**Then** la ExpiredSubscriptionRestrictionPolicy bloquea la acción.**Escenario 2****Given** el acceso bloqueado de la institución,**When** el administrador ingresa al panel,**Then** el sistema únicamente le permite acceder al módulo de facturación para regularizar el pago. | **EP01** |
| **US-10** | **Sincronización Biométrica** | Como Medical Staff, quiero sincronizar mi wearable para enviar mis métricas de estrés al motor de evaluación. | **Escenario 1****Given** el wearable emparejado por bluetooth,**When** ejecuto SyncBiometricData,**Then** el sistema de riesgo recibe la telemetría fisiológica en tiempo real.**Escenario 2****Given** una desconexión momentánea de red en el hospital,**When** la aplicación recupera la cobertura,**Then** sincroniza de inmediato todos los datos que fueron guardados de forma local en la caché. | **EP03** |
| **US-11** | **Cálculo de Puntaje de Fatiga** | Como sistema, quiero evaluar los biomarcadores para determinar la fatiga acumulada del médico. | **Escenario 1****Given** un nuevo paquete de datos biométricos procesados,**When** se activa la FatigueScoreEvaluationPolicy,**Then** el sistema recalcula y actualiza el valor del score en el Aggregate RiskAssessment.**Escenario 2****Given** lecturas inconsistentes o corruptas del sensor,**When** la política intenta computar el score,**Then** descarta la lectura errónea y mantiene el último estado válido conocido. | **EP03** |
| **US-12** | **Detección de Anomalías** | Como sistema, quiero detectar comportamientos fisiológicos atípicos para prevenir colapsos. | **Escenario 1****Given** una lectura de frecuencia cardíaca prolongada fuera del rango de seguridad,**When** actúa la BiometricAnomalyPolicy,**Then** ejecuta el comando DetectBiometricAnomaly e inserta el registro.**Escenario 2****Given** un pico de estrés alto que disminuye en menos de 30 segundos,**When** se evalúa la fluctuación,**Then** el sistema lo clasifica como evento transitorio normal y no levanta anomalías. | **EP03** |
| **US-13** | **Clasificación de Nivel de Riesgo** | Como sistema, quiero categorizar el riesgo clínico en niveles comprensibles (Bajo, Medio, Alto). | **Escenario 1****Given** el cálculo de fatiga actualizado,**When** la RiskLevelClassificationPolicy se ejecuta,**Then** clasifica el nivel del médico e impacta su Personal Risk Status View.**Escenario 2****Given** que los niveles de estrés disminuyen de manera constante tras una pausa,**When** se ejecuta el análisis,**Then** el sistema actualiza automáticamente el riesgo a un nivel inferior. | **EP03** |
| **US-14** | **Detección de Riesgo Extremo** | Como sistema, quiero identificar situaciones de agotamiento severo para disparar respuestas automáticas de protección. | **Escenario 1****Given** un nivel crítico sostenido de fatiga,**When** actúa la ExtremeRiskPolicy,**Then** emite el evento ExtremeRiskDetected para activar los protocolos de seguridad de turnos.**Escenario 2****Given** la detección de un riesgo extremo,**When** ocurre el evento en el dominio,**Then** se actualiza inmediatamente el indicador correspondiente en el Clinical Risk Dashboard general. | **EP03** |
| **US-15** | **Apertura de Incidente Clínico** | Como sistema, quiero transformar un riesgo detectado en un incidente formal gestionable por el supervisor. | **Escenario 1****Given** el evento de dominio ClinicalRiskDetected,**When** se activa la DetectedRiskIncidentPolicy,**Then** ejecuta OpenRiskIncident y crea de manera automática el Aggregate RiskIncident.**Escenario 2****Given** un incidente que ya se encuentra abierto para un médico,**When** ocurre un riesgo recurrente,**Then** el sistema anexa la información al incidente activo en lugar de duplicarlo. | **EP04** |
| **US-16** | **Asignación de Prioridad** | Como sistema, quiero priorizar los incidentes (Ej. Crítico, Advertencia) según la severidad del riesgo. | **Escenario 1****Given** un incidente recién creado derivado de un Riesgo Extremo,**When** aplica la IncidentPriorityPolicy,**Then** asigna de forma automática la etiqueta de 'Prioridad Alta'.**Escenario 2****Given** un incidente gatillado por estrés moderado,**When** la política evalúa los datos,**Then** se clasifica con prioridad de 'Observación/Preventiva'. | **EP04** |
| **US-17** | **Alerta a Supervisor** | Como sistema, quiero alertar al supervisor de turno cuando se abre un incidente para garantizar respuesta rápida. | **Escenario 1****Given** la creación de un incidente crítico,**When** actúa la SupervisorNotificationPolicy,**Then** ejecuta AlertSupervisor despachando notificaciones push y un correo vía Resend API.**Escenario 2****Given** que la alerta es procesada por el servidor,**When** el supervisor actualiza su terminal,**Then** el incidente se posiciona en el tope de su Supervisor Alert Queue. | **EP04** |
| **US-18** | **Reconocimiento de Riesgo** | Como Clinical Supervisor, quiero marcar un incidente como "En revisión" para indicar que estoy tomando cartas en el asunto. | **Escenario 1****Given** un incidente pendiente en mi lista de control,**When** ejecuto el comando AcknowledgeRisk,**Then** el estado del RiskIncident cambia de forma inmediata a 'Atendido'.**Escenario 2****Given** un incidente ya reconocido,**When** otro supervisor de la misma área revisa la herramienta,**Then** visualiza con claridad el identificador del colega que está gestionando el caso. | **EP04** |
| **US-19** | **Resolución de Incidente** | Como Clinical Supervisor, quiero cerrar un incidente una vez mitigado el riesgo del personal. | **Escenario 1****Given** que el médico involucrado ha completado su recuperación,**When** ejecuto ResolveIncident,**Then** el Aggregate RiskIncident pasa a un estado definitivo de 'Cerrado'.**Escenario 2****Given** que intento cerrar un incidente clasificado de 'Riesgo Extremo' sin haber propuesto soluciones,**When** guardo,**Then** el sistema bloquea el cierre y exige justificar la acción. | **EP04** |
| **US-20** | **Evaluación de Turno Crítico** | Como Clinical Supervisor, quiero evaluar si un turno asignado representa un peligro por la fatiga del titular. | **Escenario 1****Given** la programación médica del día,**When** ejecuto el comando EvaluateCriticalShift,**Then** la UnsafeShiftDetectionPolicy cruza los datos operativos con el estado actual de salud del médico.**Escenario 2****Given** un médico evaluado con indicadores estables en verde,**When** corre el análisis,**Then** el ShiftRiskView muestra su asignación diaria etiquetada como 'Turno Seguro'. | **EP05** |
| **US-21** | **Bloqueo de Turno** | Como sistema, quiero bloquear preventivamente la asignación de un médico exhausto para proteger a los pacientes. | **Escenario 1****Given** el evento de dominio ExtremeRiskDetected,**When** actúa de inmediato la CriticalShiftBlockingPolicy,**Then** ejecuta BlockShift y actualiza el estado de ShiftAssignment a 'Bloqueado'.**Escenario 2****Given** un turno en estado bloqueado,**When** el médico afectado intenta registrar su asistencia en la app,**Then** el sistema le deniega la acción y le muestra una instrucción preventiva. | **EP05** |
| **US-22** | **Sugerencia de Reemplazos** | Como sistema, quiero proponer a médicos disponibles y descansados cuando se bloquea un turno. | **Escenario 1****Given** que ocurre un evento ShiftBlocked,**When** se dispara la ReplacementSelectionPolicy,**Then** ejecuta SuggestReplacement poblando dinámicamente la vista Replacement Suggestions View.**Escenario 2****Given** un escenario donde no existe personal de relevo apto o disponible,**When** el sistema realiza la búsqueda,**Then** arroja un aviso crítico bajo el concepto de 'Friction Point: Replacement Not Available'. | **EP05** |
| **US-23** | **Reasignación de Carga Laboral** | Como Clinical Supervisor, quiero trasladar el turno bloqueado al reemplazo sugerido. | **Escenario 1****Given** una opción de reemplazo viable en la lista,**When** ejecuto ReassignShift,**Then** el Aggregate ShiftAssignment se actualiza y despacha el evento de dominio ShiftReassigned.**Escenario 2****Given** que concreto la reasignación en el panel,**When** el cambio se guarda en la base de datos,**Then** el sistema envía un aviso automático al nuevo médico asignado mediante la Resend Email API. | **EP05** |
| **US-24** | **Redistribución Automática** | Como sistema, quiero equilibrar la carga laboral cuando hay ausencias críticas. | **Escenario 1****Given** múltiples bloqueos de turnos en un mismo departamento,**When** actúa la WorkloadRedistributionPolicy,**Then** actualiza el Workload Distribution View para prevenir la fatiga en los suplentes.**Escenario 2****Given** un médico de reemplazo que ya ha alcanzado el límite máximo de horas de guardia semanales,**When** se balancea la carga,**Then** el sistema lo omite automáticamente de las recomendaciones. | **EP05** |
| **US-25** | **Detección de Necesidad de Descanso** | Como sistema, quiero determinar cuándo un médico necesita ser retirado temporalmente de la operación. | **Escenario 1****Given** el evento de dominio RiskIncidentOpened,**When** la política RecoveryNeedPolicy analiza el estado de fatiga del usuario,**Then** ejecuta con éxito el comando DetectRecoveryNeed.**Escenario 2****Given** que el comando de necesidad es ejecutado,**When** se confirma la condición de agotamiento,**Then** el sistema inicializa de inmediato la creación de un nuevo Aggregate RecoveryPlan. | **EP06** |
| **US-26** | **Emisión de Recomendación** | Como Clinical Supervisor, quiero sugerir un periodo de descanso específico para estabilizar al personal. | **Escenario 1****Given** el panel de control de recuperaciones,**When** ejecuto SuggestRestPeriod configurando un bloque de 45 minutos,**Then** el estado de la entidad RecoveryPlan se actualiza a 'Propuesto'.**Escenario 2****Given** que intento asignar un tiempo de pausa por debajo del protocolo básico,**When** guardo la recomendación,**Then** el sistema frena la acción y solicita ajustar el descanso a los parámetros mínimos. | **EP06** |
| **US-27** | **Notificación de Recuperación** | Como sistema, quiero avisar al personal médico que se le ha asignado un plan de recuperación. | **Escenario 1****Given** un plan en estado propuesto,**When** actúa la RecoveryNotificationPolicy,**Then** ejecuta NotifyMedicalStaff mostrando una alerta prioritaria en el terminal del médico.**Escenario 2****Given** que el usuario abre la aplicación móvil,**When** se dirige a la sección de pausas,**Then** el Recovery Recommendation View renderiza el temporizador y las instrucciones de la pausa activa. | **EP06** |
| **US-28** | **Aceptación de Plan** | Como Medical Staff, quiero aceptar la recomendación de descanso para iniciar mi recuperación fisiológica. | **Escenario 1****Given** la propuesta visible en la pantalla de mi smartphone,**When** ejecuto AcceptRecoveryPlan,**Then** el RecoveryPlan pasa a estado 'Aceptado' y mi perfil cambia momentáneamente a 'En Descanso'.**Escenario 2****Given** que el plan es aceptado por el usuario,**When** el supervisor de área examina el Recovery Status View,**Then** puede visualizar la cuenta regresiva en vivo de la pausa del profesional. | **EP06** |
| **US-29** | **Rechazo de Plan** | Como Medical Staff, quiero poder rechazar la pausa si me encuentro en medio de un procedimiento ineludible. | **Escenario 1****Given** la sugerencia en pantalla de mi terminal,**When** ejecuto RejectRecoveryPlan,**Then** el sistema interrumpe el flujo y emite el evento de dominio RecoveryPlanRejected.**Escenario 2****Given** un rechazo registrado,**When** actúa la RecoveryRefusalTrackingPolicy,**Then** escala un aviso visual de atención inmediata en la interfaz del supervisor bajo el concepto de 'Unresolved Recovery Refusal'. | **EP06** |
| **US-30** | **Confirmación de Recuperación** | Como Clinical Supervisor, quiero confirmar que el médico está listo para volver al trabajo basándome en su nueva telemetría. | **Escenario 1****Given** que las lecturas biométricas retornan a los parámetros normales de descanso,**When** ejecuto ConfirmRecovery,**Then** el sistema emite de manera oficial el evento RecoveryConfirmed.**Escenario 2****Given** que el médico continúa registrando una variabilidad cardíaca deficiente,**When** intento forzar el cierre de la recuperación,**Then** el sistema lanza una advertencia recomendando ampliar el descanso. | **EP06** |
| **US-31** | **Registro de Evaluación de Riesgo** | Como sistema, quiero auditar cada cambio de riesgo clínico para mantener trazabilidad histórica. | **Escenario 1****Given** un evento de dominio ClinicalRiskDetected,**When** actúa la RiskAssessmentAuditPolicy,**Then** ejecuta el comando RecordRiskAssessment insertando la data directamente en el AuditTrail.**Escenario 2****Given** una alta concurrencia de mediciones en el servidor,**When** se guardan las evaluaciones,**Then** el sistema garantiza el almacenamiento cronológico exacto de cada cambio de estado fisiológico. | **EP07** |
| **US-32** | **Auditoría de Decisión Crítica** | Como sistema, quiero guardar de forma inmutable todas las decisiones críticas (ej. bloqueos y rechazos) para defensa legal del hospital. | **Escenario 1****Given** que un turno es suspendido por motivos de fatiga,**When** actúa la CriticalDecisionAuditPolicy,**Then** ejecuta RecordCriticalDecision guardando un registro cifrado e inalterable en el log.**Escenario 2****Given** la generación de un evento RecoveryPlanRejected,**When** se evalúa la política de auditoría,**Then** la acción se inyecta en el historial detallando los motivos especificados por el médico. | **EP07** |
| **US-33** | **Trazabilidad de Acciones del Supervisor** | Como sistema, quiero registrar qué supervisor tomó acción ante un incidente. | **Escenario 1****Given** el evento de cierre IncidentResolved,**When** actúa la SupervisorActionAuditPolicy,**Then** ejecuta RecordSupervisorAction enlazando el UUID único del supervisor responsable.**Escenario 2****Given** que un incidente experimenta un traspaso entre áreas,**When** se guarda la auditoría,**Then** el log almacena la cadena completa de supervisores que interactuaron con el caso. | **EP07** |
| **US-34** | **Sincronización del Historial** | Como sistema, quiero actualizar de inmediato las vistas de auditoría cuando se agregan nuevos registros. | **Escenario 1****Given** nuevos comandos validados y escritos en el Aggregate de control,**When** actúa la AuditTrailSynchronizationPolicy,**Then** se actualiza en tiempo real la interfaz del Audit Log View.**Escenario 2****Given** una consulta abierta al historial por parte de gerencia,**When** entran nuevos registros de incidentes,**Then** la vista ordena la información basándose en milisegundos de ocurrencia sin necesidad de recargar. | **EP07** |
| **US-35** | **Generación de Reporte de Cumplimiento** | Como Administrador Institucional, quiero generar reportes de salud ocupacional para entidades regulatorias. | **Escenario 1****Given** un rango mensual de búsqueda seleccionado,**When** ejecuto GenerateComplianceReport,**Then** el sistema compila los datos consolidados y los expone en el Compliance Report View.**Escenario 2****Given** la información renderizada en pantalla,**When** doy clic en el botón de descarga,**Then** el sistema exporta un documento PDF estructurado y listo para auditorías institucionales de seguridad. | **EP07** |
| **US-36** | **Filtrado de Auditoría Avanzado** | Como Administrador Institucional, quiero filtrar el log de auditoría por UUID de médico para revisar su historial completo de incidencias. | **Escenario 1****Given** que accedo a la consola del Audit Log View,**When** digito el identificador o código de un médico,**Then** el sistema extrae de forma exclusiva los eventos de auditoría donde el médico es el actor principal.**Escenario 2****Given** una búsqueda con un código inexistente o erróneo,**When** aplico el filtro en la vista,**Then** la herramienta devuelve una lista limpia junto al mensaje 'Sin registros auditables en el periodo'. | **EP07** |
| **US-37** | **Cierre de Sesión Seguro** | Como usuario, quiero cerrar mi sesión explícitamente para asegurar mis datos al usar dispositivos del hospital. | **Escenario 1****Given** que me encuentro autenticado en una computadora de uso compartido,**When** ejecuto el comando SignOut,**Then** Firebase Auth destruye el token activo y bloquea la vista actual.**Escenario 2****Given** una sesión que es interrumpida automáticamente por inactividad prolongada,**When** intento interactuar con el panel,**Then** la plataforma invalida la sesión y solicita credenciales de nuevo. | **EP02** |
| **US-38** | **Consulta de Resumen de Suscripción** | Como Administrador, quiero visualizar el estado comercial de mi institución para prever pagos. | **Escenario 1****Given** que me encuentro dentro de las opciones de cuenta,**When** selecciono la opción Active Subscription Summary,**Then** la vista proyecta los datos del plan, la fecha límite de pago y el número de licencias utilizadas.**Escenario 2****Given** un inconveniente de comunicación externa con la API de Stripe,**When** consulto el resumen comercial,**Then** el sistema presenta el último estado guardado localmente junto con una advertencia de sincronización offline. | **EP01** |
| **US-39** | **Solicitud de Acceso Especial** | Como Medical Staff, quiero solicitar acceso a módulos de supervisión temporalmente si me delegan dicha responsabilidad. | **Escenario 1****Given** que cuento únicamente con rol operativo de staff,**When** ejecuto el comando RequestAccess para una sala crítica,**Then** el sistema envía una solicitud de aprobación en tiempo real al panel administrativo.**Escenario 2****Given** que el administrador valida positivamente mi petición,**When** se procesa la actualización del estado,**Then** la política me asigna el rol de supervisor auxiliar de forma temporal dentro de mi UserAccount. | **EP02** |
| **US-40** | **Filtrado de Cola de Alertas** | Como Clinical Supervisor, quiero filtrar mi Supervisor Alert Queue por prioridad para enfocarme en los riesgos extremos. | **Escenario 1****Given** múltiples incidentes listados en mi cola de control,**When** marco la casilla de filtro 'Prioridad Alta',**Then** la vista oculta las observaciones leves y renderiza únicamente las alertas de riesgo extremo.**Escenario 2****Given una cola con filtros activos,****When** presiono la opción de limpiar parámetros,**Then** la interfaz vuelve a listar el consolidado total de alertas ordenado cronológicamente. | **EP04** |
...
| **Preparada por**                        | Pareja Callaoapza, Marcelo Fausto                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
