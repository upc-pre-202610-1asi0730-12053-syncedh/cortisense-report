...
Aquí tienes la tabla estructurada exactamente con el formato solicitado, utilizando etiquetas `<p>` de HTML y negritas correspondientes para cada sección dentro de la celda de Criterios de Aceptación:

| Epic ID | Título | Descripción | Criterios de Aceptación (Mínimo 2) |
| --- | --- | --- | --- |
| **EP01** | **Gestión de Suscripciones y Planes** | Como administrador institucional, quiero gestionar los planes de pago y la activación de suscripciones para habilitar las funcionalidades contratadas por el hospital. | **Escenario 1: Activación tras confirmación de pago****Given** que el administrador ha completado el pago de un plan,**When** Stripe Sandbox confirma la transacción exitosa,**Then** la plataforma cambia el estado de la suscripción a 'Activa' y notifica al usuario.**Escenario 2: Restricción automática por expiración****Given** que la suscripción de la institución ha llegado a su fecha de vencimiento sin renovación,**When** un usuario intenta ingresar a los tableros del hospital,**Then** el sistema blocks el acceso a las herramientas premium y muestra una alerta de renovación de plan. |
| **EP02** | **Gestión de Identidad y Accesos** | Como usuario de la plataforma, quiero registrarme, autenticarme y recibir roles dentro del sistema para acceder de manera segura a mi espacio de trabajo hospitalario. | **Escenario 1: Verificación de usuario invitado****Given** que un profesional médico recibe un correo de invitación,**When** acepta los términos y completa su registro a través de Firebase Auth,**Then** el sistema lo asocia correctamente al workspace de su hospital con su estado activo.**Escenario 2: Denegación por permisos insuficientes****Given** que un usuario con el rol 'Medical Staff' intenta ingresar a la configuración de la organización,**When** el sistema evalúa su perfil,**Then** la política de accesos deniega la entrada y registra un intento no autorizado en el sistema. |
| **EP03** | **Evaluación de Riesgo Clínico** | Como sistema, quiero procesar los datos biométricos del personal médico para calcular su puntaje de fatiga e identificar riesgos clínicos inminentes. | **Escenario 1: Cálculo en tiempo real****Given** que el flujo de datos biométricos del wearable se está transmitiendo activamente,**When** el motor de análisis aplica la política de evaluación,**Then** actualiza el puntaje de fatiga y refresca el nivel de riesgo en el panel del usuario.**Escenario 2: Identificación de riesgo extremo****Given** que un médico presenta variabilidad cardíaca y cortisol fuera de los umbrales tolerables,**When** se excede el límite crítico preestablecido,**Then** el sistema emite un evento de alerta por riesgo clínico extremo. |
| **EP04** | **Gestión de Incidentes y Escalamiento** | Como supervisor clínico, quiero que el sistema registre incidentes de riesgo y me alerte de forma inmediata para poder mitigar el agotamiento del personal. | **Escenario 1: Apertura automática de incidentes****Given** que el motor de riesgo detecta una anomalía fisiológica grave en un médico,**When** se dispara el evento de riesgo clínico,**Then** el sistema abre automáticamente un incidente formal y le asigna prioridad alta.**Escenario 2: Notificación al supervisor****Given** un nuevo incidente de prioridad crítica abierto en el sistema,**When** se ejecuta la política de despacho,**Then** se envía una alerta push/email inmediata al supervisor en turno utilizando Resend Email API. |
| **EP05** | **Coordinación de Turnos** | Como supervisor clínico, quiero evaluar turnos críticos, bloquear asignaciones peligrosas y coordinar reemplazos para salvaguardar al personal y asegurar la continuidad operativa. | **Escenario 1: Bloqueo preventivo de turno****Given** que un médico programado para un turno nocturno entra en estado de riesgo extremo,**When** se evalúa la asignación del turno,**Then** el sistema bloquea automáticamente dicho turno para evitar incidentes laborales.**Escenario 2: Proposición automática de suplentes****Given** que un turno ha sido bloqueado de forma preventiva,**When** el supervisor consulta las alternativas disponibles,**Then** el sistema muestra una lista filtrada de médicos con bajo nivel de fatiga aptos para el reemplazo. |
| **EP06** | **Recuperación del Personal** | Como profesional médico o supervisor, quiero recibir y gestionar recomendaciones de descanso para recuperarme fisiológicamente de la fatiga. | **Escenario 1: Notificación de descanso sugerido****Given** que se ha detectado la necesidad imperativa de un descanso para un médico,**When** el supervisor aprueba la pausa activa,**Then** el sistema envía una alerta de recuperación directamente a la aplicación del profesional.**Escenario 2: Seguimiento de rechazo de plan****Given** que un médico decide denegar la recomendación de pausa por una urgencia,**When** presiona la opción de rechazar plan,**Then** el sistema cambia el estado del plan de recuperación a 'Rechazado' y levanta un punto de fricción para su revisión. |
| **EP07** | **Auditoría y Cumplimiento** | Como administrador institucional, quiero que el sistema guarde trazabilidad inmutable de todas las evaluaciones de riesgo y decisiones críticas para generar reportes de cumplimiento. | **Escenario 1: Sincronización inmutable del historial****Given** que un supervisor toma una decisión crítica sobre un turno o incidente,**When** la acción se completa con éxito,**Then** el sistema registra de forma automática e inmediata el evento en el log de auditoría.**Escenario 2: Consolidación de reportes institucionales****Given** el administrador necesita presentar evidencia ante un comité de salud ocupacional,**When** solicita el reporte mensual de intervenciones,**Then** el sistema procesa el historial y genera un informe de cumplimiento inalterable con estadísticas clave. |
